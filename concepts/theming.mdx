...
The library provides a `useIsDarkTheme` hook to detect the current theme:

```tsx
import { useIsDarkTheme } from '@mintlify/components';
...
<Info>
The `useIsDarkTheme` hook uses a `MutationObserver` to watch for changes to the `html` element's class list, ensuring your components stay in sync with the theme.
</Info>
